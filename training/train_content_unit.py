...
def unit_frame_accuracy(outputs: dict, batch: dict) -> torch.Tensor:
    return unit_frame_accuracy_for_logits(outputs, batch, "unit_logits")


def unit_frame_accuracy_for_logits(outputs: dict, batch: dict, logit_key: str) -> torch.Tensor:
...
def speech_unit_loss(outputs: dict, batch: dict, args) -> torch.Tensor:
    return speech_unit_loss_for_logits(outputs, batch, args, "unit_logits")


def speech_unit_loss_for_logits(outputs: dict, batch: dict, args, logit_key: str) -> torch.Tensor:
    logits = outputs.get(logit_key)
...
def build_optimizer(model: torch.nn.Module, args) -> torch.optim.Optimizer:
    ...
    return torch.optim.AdamW(
        [
            ...
            {
                "params": list(raw.geometry.parameters())
                + list(raw.geo_unit_norm.parameters())
                + list(raw.geo_unit_head.parameters()),
                "lr": args.lr * args.geometry_lr_scale,
            },
            {
    ...
    for batch in tqdm(loader, desc="train", leave=False):
        ...
        for _ in range(max(1, args.steps_per_batch)):
            ...
            with torch.amp.autocast("cuda", enabled=False):
                ...
                geo_unit_loss = speech_unit_loss_for_logits(outputs, batch, args, "geo_unit_logits")
                geo_unit_acc = unit_frame_accuracy_for_logits(outputs, batch, "geo_unit_logits")
                loss = mel_loss + unit_w * unit_loss
                if args.geometry_unit_loss_weight > 0:
                    loss = loss + float(args.geometry_unit_loss_weight) * geo_unit_loss
            ...
            totals["geo_unit"] += float(geo_unit_loss.detach().cpu())
            totals["geo_unit_acc"] += float(geo_unit_acc.detach().cpu())
...
def evaluate_content(model, loader, criterion, device, args, plot_path=None, epoch: int = 0):
    ...
    for batch in tqdm(loader, desc="val", leave=False):
        ...
        total_geo_unit += float(geo_unit_loss.detach().cpu())
        total_geo_acc += float(geo_unit_acc.detach().cpu())
        count += 1
    ...
    return {
        ...
        "geo_unit": total_geo_unit / denom,
        "geo_unit_acc": total_geo_acc / denom,
...
def run(args) -> None:
    ...
    print(
        ...
        f"energy={args.lambda_energy} unit_w={args.unit_loss_weight} "
        f"geo_unit_w={args.geometry_unit_loss_weight} unit_mismatch_w={args.unit_mismatch_loss_weight}"
    )
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        is_best = score < (best - float(args.early_stop_min_delta))
        if is_best:
            best = score
            epochs_without_improvement = 0
        ...
        row = {
            ...
            "train_geo_unit": train_metrics["geo_unit"],
            "train_geo_unit_acc": train_metrics["geo_unit_acc"],
            ...
            "val_geo_unit": None if val_metrics is None else val_metrics["geo_unit"],
            "val_geo_unit_acc": None if val_metrics is None else val_metrics["geo_unit_acc"],
            "best": best,
        ...
        val_geo_text = "n/a" if val_metrics is None else f"{val_metrics['geo_unit']:.4f}"
        val_geo_acc_text = "n/a" if val_metrics is None else f"{val_metrics['geo_unit_acc']:.3f}"
        stat_text = (
            ""
        ...
        print(
            ...
            f"geo_unit={train_metrics['geo_unit']:.4f} geo_acc={train_metrics['geo_unit_acc']:.3f} "
            ...
            f"val={val_text} val_unit={val_unit_text} val_acc={val_acc_text} "
            f"val_geo={val_geo_text} val_geo_acc={val_geo_acc_text} best={best:.6f}"
            f"{stat_text} lr=({format_lrs(optimizer)})"
            f"{' visual=frozen' if train_metrics['visual_frozen'] else ' visual=train'}{' best' if is_best else ''}"
        )
        if args.early_stop_patience > 0 and epochs_without_improvement >= args.early_stop_patience:
            print(
                f"[early-stop] no score improvement for {epochs_without_improvement} epochs "
...
def parse_args():
    ...
    parser.add_argument("--geometry-lr-scale", type=float, default=1.5)
    parser.add_argument("--encoder-lr-scale", type=float, default=1.0)
    parser.add_argument("--unit-lr-scale", type=float, default=1.0)
    parser.add_argument("--decoder-lr-scale", type=float, default=1.0)
    ...
    parser.add_argument("--geometry-mode", default="dual", choices=["basic", "tcn", "dual"])
    ...
    parser.add_argument("--unit-teacher-decay-epochs", type=int, default=40)
    parser.add_argument("--unit-teacher-min", type=float, default=0.25)
    parser.add_argument("--unit-loss-weight", type=float, default=0.15)
    parser.add_argument("--geometry-unit-loss-weight", type=float, default=0.05)
    parser.add_argument("--unit-warmup-epochs", type=int, default=0)
    parser.add_argument("--unit-ramp-epochs", type=int, default=3)
    parser.add_argument("--unit-label-smoothing", type=float, default=0.10)
    ...
    parser.add_argument("--mismatch-loss-weight", type=float, default=0.0)
    ...
    parser.add_argument("--lr-scheduler", default="plateau", choices=["constant", "onecycle", "cosine", "plateau"])
