...
def smooth_units(units: np.ndarray, window: int) -> np.ndarray:
    window = int(window)
    if window <= 1 or units.size <= 2:
        return units
    if window % 2 == 0:
        window += 1
    half = window // 2
    padded = np.pad(units.astype(np.int64), (half, half), mode="edge")
    out = units.astype(np.int64).copy()
    for i in range(out.shape[0]):
        segment = padded[i : i + window]
        labels, counts = np.unique(segment, return_counts=True)
        best = labels[counts == counts.max()]
        center = int(units[i])
        out[i] = center if center in best else int(best[0])
    return out


def transition_rate(units: torch.Tensor) -> float:
    if units.numel() <= 1:
        return 0.0
    return float((units[1:] != units[:-1]).float().mean().item())
...
def fit_files_from_subset(files: list[Path], args) -> list[Path]:
    subset = str(args.fit_subset).lower()
    if subset == "all":
        return files
    if subset != "train":
        raise ValueError(f"Unsupported fit_subset={args.fit_subset!r}")
    rng = random.Random(args.seed)
    shuffled = list(files)
    rng.shuffle(shuffled)
    val_count = max(1, int(round(len(shuffled) * float(args.fit_val_ratio)))) if len(shuffled) > 1 else 0
    fit_files = sorted(shuffled[val_count:])
    if not fit_files:
        raise RuntimeError("No files left for KMeans fit after train/val split.")
    return fit_files
...
def run(args) -> None:
    ...
    fit_files = fit_files_from_subset(files, args)

    print(f"[data] files={len(files)} fit_files={len(fit_files)} fit_subset={args.fit_subset}")
    print(f"[ssl] model={args.model_name} layer={args.layer}")
    print(
        f"[kmeans] num_units={args.num_units} sample_per_file={args.sample_per_file} "
        f"smooth_window={args.smooth_window}"
    )

    feature_chunks: list[np.ndarray] = []
    feature_dim = None
    for path in tqdm(fit_files, desc="extract-fit"):
    ...
    for src in tqdm(files, desc="write-units"):
        ...
        if args.smooth_window > 1:
            units = smooth_units(units, args.smooth_window)
        unit_t = units_to_mel_length(units, int(item["mel_len"]))
        if args.smooth_window > 1:
            unit_t = torch.from_numpy(smooth_units(unit_t.numpy(), args.smooth_window)).long()
        ...
        item["speech_unit_source"] = {
            ...
            "aligned_to": "mel_len",
            "fit_subset": str(args.fit_subset),
            "fit_val_ratio": float(args.fit_val_ratio),
        ...
        items.append(
            {
                ...
                "transition_rate": transition_rate(unit_t),
            }
        )
    ...
    manifest = {
        ...
        "transition_rate_mean": float(np.mean([x["transition_rate"] for x in items])) if items else 0.0,
        "items": items,
    }
...
def parse_args():
    ...
    parser.add_argument("--fit-subset", default="all", choices=["all", "train"])
    parser.add_argument("--fit-val-ratio", type=float, default=0.05)
