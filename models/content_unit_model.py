...
from .motion_tcn_model import LandmarkTCNTower, MotionVisualTower


class DualLipGeometryTower(nn.Module):
    """Combine compact mouth-shape features with raw landmark motion features."""

    def __init__(self, num_points: int = 40, dim: int = 128, dropout: float = 0.0):
        super().__init__()
        half = max(64, dim // 2)
        self.shape = LipGeometryTower(dim=half, dropout=dropout)
        self.motion = LandmarkTCNTower(num_points=num_points, dim=half, dropout=dropout)
    ...
    def forward(self, landmarks: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        return self.out(torch.cat([self.shape(landmarks, mask), self.motion(landmarks, mask)], dim=-1))
...
class ContentUnitLipToSpeechModel(nn.Module):
    ...
    def __init__(
        ...
        geometry_mode = str(geometry_mode).lower()
        if geometry_mode not in {"basic", "tcn", "dual"}:
            raise ValueError(f"Unsupported geometry_mode={geometry_mode!r}; expected basic, tcn, or dual.")
        self.visual = MotionVisualTower(dim=dim, spatial_tokens=spatial_tokens)
        if geometry_mode == "basic":
            self.geometry = LipGeometryTower(dim=geo_dim, dropout=dropout)
        elif geometry_mode == "tcn":
            self.geometry = LandmarkTCNTower(num_points=num_points, dim=geo_dim, dropout=dropout)
        else:
            self.geometry = DualLipGeometryTower(num_points=num_points, dim=geo_dim, dropout=dropout)
        ...
        self.geo_unit_norm = nn.LayerNorm(geo_dim)
        self.geo_unit_head = nn.Linear(geo_dim, int(num_units))
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> torch.Tensor | dict[str, torch.Tensor]:
        ...
        geo_h = self._resize_time(z_geo, target_len)
        geo_unit_logits = self.geo_unit_head(self.geo_unit_norm(geo_h))
        ...
        if bool(batch.get("return_aux", False)):
            return {
                ...
                "geo_unit_logits": geo_unit_logits,
